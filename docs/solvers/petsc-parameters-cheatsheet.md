...
Finally, you can specify any PETSc option you want with the `-A,petsc_flags` Arcane option. The flags specified by this option will overwrite the default ones (`atol`, `rtol`, `pc\_type`, `mat\_type`...). This can be useful to test some options quickly or to specify ones that are not in the `.axl` (GAMG options for example).
