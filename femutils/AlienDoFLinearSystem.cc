...
solve()
{
  ...
  // DEFINE PROFILE
  //
  for (int i = 0; i < csr_view.rows().size() - 1; i++) {
...

{
  ...
  ENUMERATE_DOF (idof, areaU.own()) {
    ...
    // writer[iIndex] = m_result_work_values[idof.localId()];
  }
}

{
  ...
  ENUMERATE_DOF (idof, areaU.own()) {
    // const Integer iIndex = allUIndex[idof->localId()];
    // writer[iIndex] = m_rhs_work_values[idof.localId()];
  }
}
