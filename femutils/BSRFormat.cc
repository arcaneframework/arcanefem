...
void BSRMatrix::initialize(Int32 block_size, Int32 nb_non_zero_value, Int32 nb_col, Int32 nb_row, bool order_values_per_block)
{
...
Real BSRMatrix::getValue(DoFLocalId row, DoFLocalId col)
{
  auto block_row = row / m_block_size;
  auto block_col = col / m_block_size;

  auto block_start = m_row_index[block_row];
  auto block_end = (block_row == m_nb_row - 1) ? m_nb_col : m_row_index[block_row + 1];

  auto row_offset = row % m_block_size;
  auto col_offset = col % m_block_size;
...
void BSRMatrix::setValue(DoFLocalId row, DoFLocalId col, Real value)
{
  auto block_row = row / m_block_size;
  auto block_col = col / m_block_size;
...
void BSRFormat::initialize(IMesh* mesh, Int32 nb_edge, bool does_linear_system_use_csr)
{
  ...
  m_bsr_matrix.initialize(nb_dof, nb_non_zero_value, nb_col, nb_node, order_values_per_block);
}

/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/

void BSRFormat::toLinearSystem(DoFLinearSystem& linear_system)
{
...
void BSRFormat::computeSparsityRowIndex()
{
  ...
  if (m_use_csr_in_linear_system) {
    info() << "BSRFormat(computeSparsityRowIndex): Compute nb_nz_per_row BSR matrix array";
