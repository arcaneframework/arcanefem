...
//
// ----------------------------------------------------------------------------
// Comment    : simple plate problem mesh for double couple
//
// Parameters : length  - this is the length of the plate
//              width   - this is the width of the plate
//
// Usage      : gmsh square_double-couple.geo -setnumber length 50.0 -2 -format msh41 -algo quadqs
//
//
//-----------------------------------------------------------------------------
...
Point(1) = {0.0    , 0.0   , 0.0 , 1.5};
Point(2) = {length , 0.0   , 0.0 , 1.5};
Point(3) = {length , width , 0.0 , 1.5};
Point(4) = {0.0    , width , 0.0 , 1.5};
Point(5) = {length/2., width/2. - h, 0, 4.5};
Point(6) = {length/2., width/2. + h, 0, 4.5};
Point(7) = {length/2. + h, width/2., 0, 4.5};
Point(8) = {length/2. - h, width/2., 0, 4.5};
Point(9) = {length/2., width/2., 0, 4.5};
