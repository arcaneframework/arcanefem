...
This XML configuration file (e.g `inputs/direct.arc`) is used for setting up a Finite Element Method (FEM) simulation in ArcaneFEM. Below is a detailed breakdown of each section in the configuration.
...
- **Dirichlet Boundary Condition:** Defines the boundary condition on the specified surface (`boundary`) with a given value (`145.5 NULL`).
...
The tests are present in the form of `.arc` files:
