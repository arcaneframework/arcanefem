...
{
  RealMatrix<1, 6> dxUx = { dxu[0], 0., dxu[1], 0., dxu[2], 0. };
  RealMatrix<1, 6> dyUx = { dyu[0], 0., dyu[1], 0., dyu[2], 0. };
  RealMatrix<1, 6> dxUy = { 0., dxu[0], 0., dxu[1], 0., dxu[2] };
  RealMatrix<1, 6> dyUy = { 0., dyu[0], 0., dyu[1], 0., dyu[2] };
...
ARCCORE_HOST_DEVICE RealMatrix<2, 6> computeElementVectorTria3Gpu(CellLocalId cell_lid, const IndexedCellNodeConnectivityView& cn_cv, const Accelerator::VariableNodeReal3InView& in_node_coord, Real lambda, Real mu, Int32 node_lid)
{
  ...
  // ∫∫ (λ+2μ)(∂𝑢𝑥/∂𝑥 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑦/∂𝑦 ∂𝑣𝑦/∂𝑦)
  RealMatrix<1, 6> normal_strain_energy_x = (lambda + 2 * mu) * ((dxUx(0,node_lid*2) * dxUx) + (dyUy(0,node_lid*2) * dyUy)) * area;
  RealMatrix<1, 6> normal_strain_energy_y = (lambda + 2 * mu) * ((dxUx(0,node_lid*2+1) * dxUx) + (dyUy(0,node_lid*2+1) * dyUy)) * area;

  // ∫∫ λ(∂𝑢𝑦/∂𝑦 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑥/∂𝑥 ∂𝑣𝑦/∂𝑦)
  RealMatrix<1, 6> compressibility_effect_x = (lambda) * ((dyUy(0,node_lid*2) * dxUx) + (dxUx(0,node_lid*2) * dyUy)) * area;
  RealMatrix<1, 6> compressibility_effect_y = (lambda) * ((dyUy(0,node_lid*2+1) * dxUx) + (dxUx(0,node_lid*2+1) * dyUy)) * area;

  // ∫∫ μ(∂𝑢𝑦/∂𝑥 + ∂𝑢𝑥/∂𝑦)(∂𝑣𝑥/∂𝑦 + ∂𝑣𝑦/∂𝑥)
  RealMatrix<1, 6> shear_energy_x = (mu) * ((dxUy(0,node_lid*2) + dyUx(0,node_lid*2)) * (dyUx + dxUy)) * area;
  RealMatrix<1, 6> shear_energy_y = (mu) * ((dxUy(0,node_lid*2+1) + dyUx(0,node_lid*2+1)) * (dyUx + dxUy)) * area;

  RealMatrix <1, 6> result_x = normal_strain_energy_x + compressibility_effect_x + shear_energy_x;
  RealMatrix <1, 6> result_y = normal_strain_energy_y + compressibility_effect_y + shear_energy_y;
...
{
  RealMatrix<1, 12> dxUx = { dxu[0], 0., 0.,    dxu[1], 0., 0.,    dxu[2], 0., 0.,    dxu[3], 0., 0. };
  RealMatrix<1, 12> dyUx = { dyu[0], 0., 0.,    dyu[1], 0., 0.,    dyu[2], 0., 0.,    dyu[3], 0., 0. };
  RealMatrix<1, 12> dzUx = { dzu[0], 0., 0.,    dzu[1], 0., 0.,    dzu[2], 0., 0.,    dzu[3], 0., 0. };

  RealMatrix<1, 12> dxUy = { 0., dxu[0], 0.,    0., dxu[1], 0.,    0., dxu[2], 0.,    0., dxu[3], 0. };
  RealMatrix<1, 12> dyUy = { 0., dyu[0], 0.,    0., dyu[1], 0.,    0., dyu[2], 0.,    0., dyu[3], 0. };
  RealMatrix<1, 12> dzUy = { 0., dzu[0], 0.,    0., dzu[1], 0.,    0., dzu[2], 0.,    0., dzu[3], 0. };

  RealMatrix<1, 12> dxUz = { 0., 0., dxu[0],    0., 0., dxu[1],    0., 0., dxu[2],    0., 0., dxu[3] };
  RealMatrix<1, 12> dyUz = { 0., 0., dyu[0],    0., 0., dyu[1],    0., 0., dyu[2],    0., 0., dyu[3] };
  RealMatrix<1, 12> dzUz = { 0., 0., dzu[0],    0., 0., dzu[1],    0., 0., dzu[2],    0., 0., dzu[3] };
...
ARCCORE_HOST_DEVICE RealMatrix<3, 12> computeElementVectorTetra4Gpu(CellLocalId cell_lid, const IndexedCellNodeConnectivityView& cn_cv, const Accelerator::VariableNodeReal3InView& in_node_coord, Real lambda, Real mu, Int32 node_lid)
{
 ...
 //            ∂𝑢𝑦/∂𝑦 ∂𝑣𝑦/∂𝑦 +
 //            ∂𝑢𝑧/∂𝑧 ∂𝑣𝑧/∂𝑧 )
 RealMatrix<1, 12> normal_strain_energy_x = (lambda + 2 * mu) * ( (dxUx(0,node_lid*3) * dxUx) +
                                                                   (dyUy(0,node_lid*3) * dyUy) +
                                                                   (dzUz(0,node_lid*3) * dzUz) ) * volume;

 RealMatrix<1, 12> normal_strain_energy_y = (lambda + 2 * mu) * ( (dxUx(0,node_lid*3+1) * dxUx) +
                                                                   (dyUy(0,node_lid*3+1) * dyUy) +
                                                                   (dzUz(0,node_lid*3+1) * dzUz) ) * volume;

 RealMatrix<1, 12> normal_strain_energy_z = (lambda + 2 * mu) * ( (dxUx(0,node_lid*3+2) * dxUx) +
  ...
  //     + ∂𝑢𝑧/∂𝑧 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑥/∂𝑥 ∂𝑣𝑧/∂𝑧
  //     + ∂𝑢𝑦/∂𝑦 ∂𝑣𝑧/∂𝑧 + ∂𝑢𝑧/∂𝑧 ∂𝑣𝑦/∂𝑦)
  RealMatrix<1, 12> compressibility_effect_x = (lambda) * ((dyUy(0,node_lid*3) * dxUx) + (dxUx(0,node_lid*3) * dyUy) +
                                                            (dzUz(0,node_lid*3) * dxUx) + (dxUx(0,node_lid*3) * dzUz) +
                                                            (dyUy(0,node_lid*3) * dzUz) + (dzUz(0,node_lid*3) * dyUy) ) * volume;

  RealMatrix<1, 12> compressibility_effect_y = (lambda) * ((dyUy(0,node_lid*3+1) * dxUx) + (dxUx(0,node_lid*3+1) * dyUy) +
                                                            (dzUz(0,node_lid*3+1) * dxUx) + (dxUx(0,node_lid*3+1) * dzUz) +
                                                            (dyUy(0,node_lid*3+1) * dzUz) + (dzUz(0,node_lid*3+1) * dyUy) ) * volume;

  RealMatrix<1, 12> compressibility_effect_z = (lambda) * ((dyUy(0,node_lid*3+2) * dxUx) + (dxUx(0,node_lid*3+2) * dyUy) +
  ...
  //     μ(∂𝑢𝑧/∂𝑦 + ∂𝑢𝑦/∂𝑧)(∂𝑣𝑦/∂𝑧 + ∂𝑣𝑧/∂𝑦) +
  //     μ(∂𝑢𝑥/∂𝑧 + ∂𝑢𝑧/∂𝑥)(∂𝑣𝑧/∂𝑥 + ∂𝑣𝑥/∂𝑧)
  RealMatrix<1, 12> shear_energy_x = (mu) * ( ((dxUy(0,node_lid*3) + dyUx(0,node_lid*3)) * (dyUx + dxUy)) +
                                               ((dzUy(0,node_lid*3) + dyUz(0,node_lid*3)) * (dyUz + dzUy)) +
                                               ((dxUz(0,node_lid*3) + dzUx(0,node_lid*3)) * (dzUx + dxUz)) ) * volume;

  RealMatrix<1, 12> shear_energy_y = (mu) * ( ((dxUy(0,node_lid*3+1) + dyUx(0,node_lid*3+1)) * (dyUx + dxUy)) +
                                               ((dzUy(0,node_lid*3+1) + dyUz(0,node_lid*3+1)) * (dyUz + dzUy)) +
                                               ((dxUz(0,node_lid*3+1) + dzUx(0,node_lid*3+1)) * (dzUx + dxUz)) ) * volume;



  RealMatrix<1, 12> shear_energy_z = (mu) * ( ((dxUy(0,node_lid*3+2) + dyUx(0,node_lid*3+2)) * (dyUx + dxUy)) +
                                               ((dzUy(0,node_lid*3+2) + dyUz(0,node_lid*3+2)) * (dyUz + dzUy)) +
                                               ((dxUz(0,node_lid*3+2) + dzUx(0,node_lid*3+2)) * (dzUx + dxUz)) ) * volume;


  RealMatrix <1, 12> result_x = normal_strain_energy_x + compressibility_effect_x + shear_energy_x;
  RealMatrix <1, 12> result_y = normal_strain_energy_y + compressibility_effect_y + shear_energy_y;
  RealMatrix <1, 12> result_z = normal_strain_energy_z + compressibility_effect_z + shear_energy_z;
