...
Under small elastic deformation, the steady elastic deformation equation (nonlinear elastic system) on  domain $\Omega$ reads
...
here, $\lambda\in\mathbb{R}^{+}$ and $\mu\in\mathbb{R}^{+}$ are the Lame's elastoplasticity parameters for the homogeneous material, $\mathbb{I}$ is the identity tensor, and $\mathbf{u}(\mathbf{x})$ is the displacement field vector. This governing PDE iis also knows as Navier's equation.
...
We will provide a simple 2D bar problem example. The physics solved is a bar that bends under its own load. This XML configuration file is used for setting up an elastoplasticity problem simulation in ArcaneFEM. Below is a detailed explanation of each section in the configuration for one of the tests `bar.2D.Dirichlet.bodyForce.arc`.
