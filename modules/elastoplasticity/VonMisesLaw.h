...
inline void FemModuleElastoplasticity::_updateGlobalTangentMaterialTensorVonMisesTria3Cpu()
{
  ENUMERATE_ (Cell, icell, allCells())
  {
    ...
    for (Int8 iGP = 0; iGP < m_nGP; ++iGP ) {
      ...
      m_C_tang_gp(cell, iGP, 0, 0) = m_C_elas_2d(0, 0) + C_tang_update(0, 0);
      m_C_tang_gp(cell, iGP, 0, 1) = m_C_elas_2d(0, 1) + C_tang_update(0, 1);
      m_C_tang_gp(cell, iGP, 0, 2) = m_C_elas_2d(0, 2) + C_tang_update(0, 2);

      m_C_tang_gp(cell, iGP, 1, 0) = m_C_tang_gp(cell, iGP, 0, 1);
      m_C_tang_gp(cell, iGP, 1, 1) = m_C_elas_2d(1, 1) + C_tang_update(1, 1);
      m_C_tang_gp(cell, iGP, 1, 2) = m_C_elas_2d(1, 2) + C_tang_update(1, 2);

      m_C_tang_gp(cell, iGP, 2, 0) = m_C_tang_gp(cell, iGP, 0, 2);
      m_C_tang_gp(cell, iGP, 2, 1) = m_C_tang_gp(cell, iGP, 1, 2);
      m_C_tang_gp(cell, iGP, 2, 2) = m_C_elas_2d(2, 2) + C_tang_update(2, 2);
    }
  }
}
...
inline RealMatrix<3, 3> FemModuleElastoplasticity::_updateGlobalTangentMaterialTensorVonMisesTria3CpuBase(const Cell& cell, Int8& iGP)
{
  ...
  RealMatrix<3, 3> C_tang_update;
  C_tang_update(0, 0) = - tangentA * flowN_xx * flowN_xx - 4. * mu * beta / 3.;
  C_tang_update(0, 1) = - tangentA * flowN_xx * flowN_yy + 2. * mu * beta / 3.;
  C_tang_update(0, 2) = - tangentA * flowN_xx * flowN_xy;

  C_tang_update(1, 0) = C_tang_update(0, 1);
  C_tang_update(1, 1) = - tangentA * flowN_yy * flowN_yy - 4. * mu * beta / 3.;
  C_tang_update(1, 2) = - tangentA * flowN_yy * flowN_xy;

  C_tang_update(2, 0) = C_tang_update(0, 2);
  C_tang_update(2, 1) = C_tang_update(1, 2);
  C_tang_update(2, 2) = - tangentA * flowN_xy * flowN_xy - 2. * mu * beta;

  return C_tang_update;
}
