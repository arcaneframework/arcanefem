...
//-----------------------------------------------------------------------------
/*---------------------------------------------------------------------------*/
/* DruckerPragerLaw.cc                                         (C) 2000-2026 */
/*                                                                           */
...
_computeLocalDruckerPragerElementMatrixTria3Cpu(Cell cell, bool assemble_elastic)
{
...
computeLocalDruckerPragerElementMatrixTria3Gpu(CellLocalId cell_lid,
...
computeLocalDruckerPragerElementVectorTria3Gpu(CellLocalId cell_lid,
                                               const IndexedCellNodeConnectivityView& cn_cv,
                                               const Accelerator::VariableNodeReal3InView& in_node_coord,
                                               const Accelerator::VariableNodeReal3InView& in_DUn,
                                               const Accelerator::VariableNodeReal3InView& in_U,
...
_assembleBilinearOperatorLocalDruckerPrager(bool elastic_assembly)
{
  ...
  if (m_matrix_format == "BSR") {
    ...
    if (mesh()->dimension() == 2) {
      ...
      [=] ARCCORE_HOST_DEVICE(CellLocalId cell_lid) { return computeLocalDruckerPragerElementMatrixTria3Gpu(cell_lid, cn_cv, in_node_coord,
                                                                                                            in_DUn, in_U,
    ...
    if (mesh()->dimension() == 2) {
      ...
      [=] ARCCORE_HOST_DEVICE(CellLocalId cell_lid, Int32 node_lid) { return computeLocalDruckerPragerElementVectorTria3Gpu(cell_lid, cn_cv, in_node_coord,
                                                                                                                            in_DUn, in_U,
                                                                                                                            in_eps_p_old_gp, in_eps_p_zz_old_gp,
                                                                                                                            C_elas_2d,
                                                                                                                            in_bulk, in_dpEta, in_dpC, in_mu,
