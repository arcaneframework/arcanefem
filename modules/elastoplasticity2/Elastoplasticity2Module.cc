...
//-----------------------------------------------------------------------------
/*---------------------------------------------------------------------------*/
/* Elastoplasticity2Module.cc                                  (C) 2000-2026 */
/*                                                                           */
...
Elastoplasticity2Module::
Elastoplasticity2Module(const ModuleBuildInfo& mbi)
: ArcaneElastoplasticity2Object(mbi)
...
template <int N> void Elastoplasticity2Module::
 ...
 */
/*---------------------------------------------------------------------------*/
inline Real Elastoplasticity2Module::
...
_normL2(VariableDoFReal& u, const IndexedNodeDoFConnectivityView& node_dof)
{
  ...
  Int32 mesh_dimension = mesh()->dimension();
  {
    ENUMERATE_ (Node, inode, ownNodes()) {
      ...
      if (mesh_dimension == 2) {
        norm_residual = math::pow(u[node_dof.dofId(inode, 0)], 2.0) + math::pow(u[node_dof.dofId(inode, 1)], 2.0);
      }
      else {
        norm_residual = math::pow(u[node_dof.dofId(inode, 0)], 2.0) + math::pow(u[node_dof.dofId(inode, 1)], 2.0) + math::pow(u[node_dof.dofId(inode, 2)], 2.0);
      }
...
_normL1(VariableDoFReal& u, const IndexedNodeDoFConnectivityView& node_dof)
{
  ...
  Int32 mesh_dimension = mesh()->dimension();
  {
    ENUMERATE_ (Node, inode, ownNodes()) {
      Real norm_residual = 0.0;
      if (mesh_dimension == 2) {
        norm_residual = u[node_dof.dofId(inode, 0)] + u[node_dof.dofId(inode, 1)];
      }
      else {
        norm_residual = u[node_dof.dofId(inode, 0)] + u[node_dof.dofId(inode, 1)] + u[node_dof.dofId(inode, 2)];
      }
...
ARCANE_REGISTER_MODULE_ELASTOPLASTICITY2(Elastoplasticity2Module);
