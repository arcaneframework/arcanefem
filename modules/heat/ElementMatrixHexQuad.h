...
/*                                                                           */
/* Contains functions to compute the FEM element matrices for Heat Eq.       */
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
 ...
 *       𝑎(𝑢,𝑣) = ∫∫ λ(∂𝑢/∂𝑥 ∂𝑣/∂𝑥  + ∂𝑢/∂𝑦 ∂𝑣/∂𝑦)dΩ + ∫∫ (𝑢𝑣/δ𝑡)dΩ
 *
 * Steps involved:
  ...
  for (Int8 ixi = 0; ixi < 2; ++ixi) {
    for (Int8 ieta = 0; ieta < 2; ++ieta) {
      ...
      ae += (dxU ^ dxU) * integration_weight * lambda + (dyU ^ dyU) * integration_weight * lambda +
            (N ^ N) * integration_weight / dt; // mass matrix contribution
    }
  }
  return ae;
}
 ...
 *
 * This function calculates the integral of:
 *       𝑎(𝑢,𝑣) = ∫∫∫ λ(∂𝑢/∂𝑥 ∂𝑣/∂𝑥 + ∂𝑢/∂𝑦 ∂𝑣/∂𝑦 + ∂𝑢/∂𝑧 ∂𝑣/∂𝑧)dΩ + ∫∫∫ (𝑢𝑣/δ𝑡)dΩ
 *
 * Steps involved:
...
RealMatrix<8, 8> FemModuleHeat::_computeElementMatrixHexa8(Cell cell)
{
