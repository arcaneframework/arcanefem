...
startInit(){
  ...
  if (szType.contains("young")) elast_type = Types::YoungNu;
  else if (szType.contains("lame")) elast_type = Types::Lame;
  else if (szType.contains("vel")) elast_type = Types::Veloc;
  else {
...
_initCells(){
  ...
  ENUMERATE_CELL (icell, allCells()) {
    Real vp{0.}, vs{0.}, E{0.}, nu{0.}, lambda{0.}, mu{0.};
    const Cell& cell = *icell;
    auto rho = m_rho[cell];
    if (elast_type == Types::YoungNu) {
      E = m_young[cell];
      nu = m_nu[cell];
    ...
    } else if (elast_type == Types::Veloc) {
      vp = m_vp[cell];
      vs = m_vs[cell];
      mu = rho*vs*vs;
