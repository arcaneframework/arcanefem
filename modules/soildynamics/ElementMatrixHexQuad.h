...
_computeElementMatrixQuad4(Cell cell)
{
  ...
  for (Int8 ixi = 0; ixi < 2; ++ixi) {
    for (Int8 ieta = 0; ieta < 2; ++ieta) {
      ...
      const auto gp_info =
      ArcaneFemFunctions::FeOperation2D::computeGradientsAndJacobianQuad4(cell, m_node_coord, xi, eta);
      ...
      ae += c0 * ((Nx ^ Nx) + (Ny ^ Ny)) * weight //   ∫∫ (c₀)(𝐮 ⋅ 𝐯) dΩ
      + c1 * ((dyUy ^ dxUx) + (dxUx ^ dyUy)) * weight // + ∫∫ (c₁)(∂𝑢𝑦/∂𝑦 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑥/∂𝑥 ∂𝑣𝑦/∂𝑦)
      + (2 * c2 + c1) * ((dxUx ^ dxUx) + (dyUy ^ dyUy)) * weight // + ∫∫ (c₁+2c₂)(∂𝑢𝑥/∂𝑥 ∂𝑣𝑥/∂𝑥 + ∂𝑢𝑦/∂𝑦 ∂𝑣𝑦/∂𝑦)
      + c2 * ((dxUy + dyUx) ^ (dyUx + dxUy)) * weight; // + ∫∫ (c₂)(∂𝑢𝑦/∂𝑥 + ∂𝑢𝑥/∂𝑦)(∂𝑣𝑥/∂𝑦 + ∂𝑣𝑦/∂𝑥)
    }
  }
  ...
  for (Int8 ixi = 0; ixi < 2; ++ixi) {
    for (Int8 ieta = 0; ieta < 2; ++ieta) {
      for (Int8 izeta = 0; izeta < 2; ++izeta) {
        ...
        const auto gp_info =
        ArcaneFemFunctions::FeOperation3D::computeGradientsAndJacobianHexa8(cell, m_node_coord, xi, eta, zeta);
        ...
        ae += c0 * ((Nx ^ Nx) + (Ny ^ Ny) + (Nz ^ Nz)) * weight + (c1) * ((dxUx ^ dxUx) + (dyUy ^ dyUy) + (dzUz ^ dzUz) + (dyUy ^ dxUx) + (dxUx ^ dyUy) + (dzUz ^ dxUx) + (dxUx ^ dzUz) + (dyUy ^ dzUz) + (dzUz ^ dyUy)) * weight +
        (c2) * (2. * ((dxUx ^ dxUx) + (dyUy ^ dyUy) + (dzUz ^ dzUz)) + (((dxUy + dyUx) ^ (dyUx + dxUy)) + ((dzUy + dyUz) ^ (dyUz + dzUy)) + ((dxUz + dzUx) ^ (dzUx + dxUz)))) * weight;
      }
    }
  }
