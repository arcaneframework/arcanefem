...
class AlephFemLinearSystemDoFImpl
...
, public FemLinearSystemDoFImpl
{
 public:
  ...
  AlephFemLinearSystemDoFImpl(ISubDomain* sd, const Arcane::VariableDoFReal& dof_variable)
 ...
 private:
  ...
  void _computeMatrixInfo()  {
    ...
    //m_aleph_kernel->initialize(total_nb_dofs, nb_dofs);
  ...
  ENUMERATE_ (DoF, idof, own_dofs) {
    DoF dof = *idof;
    m_dof_matrix_indexes[dof] += row_offset;
    info() << "ROW2=" << m_dof_matrix_indexes[dof];
  }
#endif
  ...
  void solve() override  {
    ...
    ENUMERATE_ (DoF, idof, own_dofs) {
      ...
      if (do_verbose)
        info() << "DoF uid=" << dof.uniqueId() << " V=" << aleph_result[index] << " U=" << m_dof_variable[dof]
...
createAlephFemLinearSystemDoFImpl(ISubDomain* sd, const Arcane::VariableDoFReal& dof_variable){
  auto* x = new AlephFemLinearSystemDoFImpl(sd, dof_variable);
  x->build();
  return x;
}
